...
This release adds local durable SQL persistence and database migrations while preserving the mock-only FastAPI foundation. No live integrations are connected. SQLite remains development-only, migrations are explicit, retention cleanup is dry-run by default, and duplicate approval execution is transactionally prevented.
