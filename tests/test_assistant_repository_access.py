...
def test_repository_status_succeeds_for_authenticated_users(
    tmp_path: Path,
) -> None:
...
def test_repository_endpoints_require_authentication(
    tmp_path: Path,
) -> None:
    repo = make_repo(tmp_path)
...
def test_file_listing_excludes_ignored_paths_and_filters(
    tmp_path: Path,
) -> None:
    repo = make_repo(tmp_path)
...
def test_secret_database_traversal_absolute_and_oversized_reads_rejected(
    tmp_path: Path,
) -> None:
    repo = make_repo(tmp_path)
...
def test_safe_source_file_can_be_read(
    tmp_path: Path,
) -> None:
    repo = make_repo(tmp_path)
...
def test_filename_content_extension_and_limit_search(
    tmp_path: Path,
) -> None:
    repo = make_repo(tmp_path)
...
def test_selected_files_loaded_into_chat_context_and_persisted(
    tmp_path: Path,
) -> None:
    repo = make_repo(tmp_path)
...
def test_no_repository_write_or_network_occurs(
    tmp_path: Path,
) -> None:
    repo = make_repo(tmp_path)
